...
<Default title={frontmatter.meta[0].title}>
    ...
    <div className='flex justify-center !-my-8 h-[800px]'>
    ...
    Composing objects together which have themselves been composed of other objects is a powerful way to build complex systems with a high degree of flexibility and modularity.
    ...
    <div className='float-right !-mt-4 !-mb-8'>
        <Player project='003-mario' auto />
    </div>

    ## Inheritance

    What can we now learn about inheritance from retro games?

    Let's think about... Mario!

    In most Mario games, when  Mario obtains a Power Up  (such as a <img src="/003/fire-flower.png" className='inline size-7 align-text-bottom !m-0' /> Fire Flower or a <img src="/003/super-mushroom.png" className='inline size-7 align-text-bottom !m-0' /> Super Mushroom) he gains a specific ability or enhancement.
    ...
    <div className='flex flex-row'>
        <div className='grow'>Super Mario _is a_ Mario.</div>
        <div className='grow text-right'>Fire Mario _is a_ Mario.</div>
    </div>

    And so on.

    Inheritance is when we design our types around what they are.

    Inheritance can be a nice way to model a simple system. So, how come people often say we should: _"Favour Composition over Inheritance?"_

    The main arguments for choosing Composition over Inheritance primarily revolve around maintainability and flexibility.
    ...
    Let's explore this using the ghosts from Pacman.

    Imagine a ghost base class with four other ghost subclasses.

    If we were to introduce a change to the ghost base class - all subclasses would be impacted.

    Composition however provides us with object containment which will reduce the impact of the change.

    In the simple example, inheritance was changed into composition with Dependency Injection.

    Earlier, we spoke about Single Responsibility.

    Two of the five solid principles.

    I'll be covering the solid principles and how Composition helps us write testable code in the future...

    but for now - seems like it's game over for inheritance, right?
    ...
    But not all inheritance is bad and we should not blindly avoid it.

    Composition and Inheritance are just tools.

    Instead, I love this take from Mathias Verraes on defining heuristics on when and when not to use Inheritance.
